-- Create deterministic vendor selection function
...
BEGIN
  ...
  LOOP
    ...
    RAISE LOG 'Processando vendedor %: % agendamentos, %% conversão', 
      v_vendedor.name, v_agendamentos_count, v_taxa_conversao;
    ...
    IF v_agendamentos_count < v_menor_agendamentos THEN
      ...
      RAISE LOG 'Novo melhor vendedor (menor agendamentos): % (% agendamentos)', 
        v_vendedor.name, v_agendamentos_count;
    ...
    ELSIF v_agendamentos_count = v_menor_agendamentos AND v_taxa_conversao > v_maior_taxa THEN
      v_maior_taxa := v_taxa_conversao;
      v_melhor_vendedor := v_vendedor;
      RAISE LOG 'Novo melhor vendedor (maior conversão): % (%% conversão)', 
        v_vendedor.name, v_taxa_conversao;
    ...
    ELSIF v_agendamentos_count = v_menor_agendamentos AND v_taxa_conversao = v_maior_taxa AND v_melhor_vendedor IS NULL THEN
      v_melhor_vendedor := v_vendedor;
      RAISE LOG 'Vendedor selecionado por ordem alfabética: %', v_vendedor.name;
    END IF;
  END LOOP;
  ...
  -- Log final
  RAISE LOG 'Seleção concluída. Melhor vendedor: %, Diagnóstico: %', 
    COALESCE(v_melhor_vendedor.name, 'NENHUM'), v_diagnostico;
