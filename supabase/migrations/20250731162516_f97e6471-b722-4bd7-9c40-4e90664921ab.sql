-- Inserir vendedores de teste se não existirem
INSERT INTO profiles (id, name, email, user_type, nivel, ativo)
SELECT 
  gen_random_uuid(),
  'Vendedor Teste ' || generate_series,
  'vendedor' || generate_series || '@teste.com',
  'vendedor',
  CASE 
    WHEN generate_series % 3 = 0 THEN 'senior'
    WHEN generate_series % 3 = 1 THEN 'pleno' 
    ELSE 'junior'
  END,
  true
FROM generate_series(1, 5)
WHERE NOT EXISTS (
  SELECT 1 FROM profiles WHERE email LIKE '%@teste.com'
);
...
-- Inserir vendas aleatórias para as últimas 3 semanas
WITH vendedores_teste AS (
  SELECT id FROM profiles WHERE email LIKE '%@teste.com' AND user_type = 'vendedor'
),
cursos_teste AS (
...
vendas_aleatorias AS (
  SELECT 
    ...
    (SELECT id FROM vendedores_teste ORDER BY RANDOM() LIMIT 1) as vendedor_id,
  ...
  FROM semanas s
  CROSS JOIN generate_series(1, 8) -- 8 vendas por semana
)
...
reunioes_aleatorias AS (
  SELECT 
    ...
    (SELECT id FROM vendedores_teste ORDER BY RANDOM() LIMIT 1) as vendedor_id,
    (SELECT id FROM sdrs_teste ORDER BY RANDOM() LIMIT 1) as sdr_id,
    ...
    NOW() as updated_at
  FROM semanas s
  CROSS JOIN generate_series(1, 5) -- 5 reuniões por semana
)
