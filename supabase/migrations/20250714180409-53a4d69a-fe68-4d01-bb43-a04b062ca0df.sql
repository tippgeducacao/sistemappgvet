-- Expandir tabela niveis_vendedores para incluir todos os níveis de SDR
-- Primeiro, alterar o enum do campo nivel para incluir todos os tipos
ALTER TABLE public.niveis_vendedores 
DROP CONSTRAINT niveis_vendedores_nivel_check;

-- Adicionar todos os níveis incluindo SDRs com modalidade e nível
ALTER TABLE public.niveis_vendedores 
...
-- Limpar registros de SDR antigos
DELETE FROM public.niveis_vendedores WHERE tipo_usuario = 'sdr';

-- Inserir níveis detalhados para SDRs baseados na imagem
-- SDR Inbound
...
COMMENT ON COLUMN public.niveis_vendedores.nivel IS 'Nível: junior, pleno, senior (vendedores) ou sdr_inbound_junior, sdr_inbound_pleno, sdr_inbound_senior, sdr_outbound_junior, sdr_outbound_pleno, sdr_outbound_senior (SDRs)';
