    ...
    <div className="fixed inset-0 z-50 min-h-screen bg-background overflow-hidden">
      ...
      >
        <div className="max-w-full mx-auto overflow-hidden">
          ...
          {/* SEÇÃO DA PLANILHA */}
          <div className="mt-8">
            <h2 className="text-xl font-bold text-foreground mb-4 text-center">Planilha Detalhada</h2>
            
            {/* Tabela de Vendedores */}
            <div className="mb-8">
              <h3 className="text-lg font-semibold text-foreground mb-4">Vendedores</h3>
              <div className="overflow-x-auto bg-card/50 backdrop-blur-sm rounded-lg border border-white/20">
                <table className="w-full text-sm">
                  <thead className="bg-primary/10">
                    <tr>
                      ...
                      {getWeeksOfMonth(getMesAnoSemanaAtual().ano, getMesAnoSemanaAtual().mes).map(week => (
                  ...
                  <tbody>
                    {vendedoresOnly.map((vendedor, index) => {
                      const vendedorData = vendedores.find(v => v.id === vendedor.id);
                      const vendedorNivel = vendedorData?.nivel || 'junior';
                      const nivelConfig = niveis.find(n => n.nivel === vendedorNivel);
                      const weeks = getWeeksOfMonth(getMesAnoSemanaAtual().ano, getMesAnoSemanaAtual().mes);
                      const weeklyPoints = getVendedorWeeklyPoints(vendedor.id, weeks);
                      const totalPoints = weeklyPoints.reduce((sum, points) => sum + points, 0);
                      const metaMensal = (nivelConfig?.meta_semanal_vendedor || 6) * weeks.length;
                      const achievementPercentage = metaMensal > 0 ? (totalPoints / metaMensal) * 100 : 0;
